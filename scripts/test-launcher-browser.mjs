...
try {
 ...
 assert.ok((await get()).top<info.top-60);const position=await evaluate(sw,`chrome.storage.local.get('draLauncherPosition')`);assert.equal(typeof position.draLauncherPosition.x,'number');assert.equal(typeof position.draLauncherPosition.y,'number');assert.ok((await get()).x<info.x-60);
 ...
 await waitFor(async()=>(await get()).visible,'launcher stays visible while panel open');
 ...
 await evaluate(sw,`launcherTest.set({status:'stopped',endedAt:new Date(Date.now()-9000).toISOString()})`);await waitFor(async()=>(await get()).visible,'finished remains visible');
 report.scenarios.push('ordinary webpage content injection','drag persists position','dark theme','trusted click opens native side panel','closing native panel preserves running task','paused status','ended launcher remains available');
