...
热门卡片已有分区名时，时长、播放或卡片上的分区名、关键词已直接淘汰的视频 SHALL NOT 请求视频详情或粉丝数。推荐流卡片没有分区名时 SHALL 先请求视频详情，再做规则判断；这次才淘汰的不再请求粉丝数。未对上淘汰或理想规则、交给人工的视频 SHALL 仍补抓粉丝数。采集 SHALL NOT 请求作者投稿列表。
...
推荐流 SHALL 先请求 `/x/web-interface/wbi/view` 取得分区名，再做规则判断。热门在卡片尚未直接淘汰时才请求该详情。请求前 SHALL NOT 等待。仍命中或待确认时，再请求 `/x/relation/stat` 取粉丝数，请求前同样 SHALL NOT 等待。
