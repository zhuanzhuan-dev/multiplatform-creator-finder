...
const platforms = [{id:'douyin',label:'抖音推荐',unit:'条'}, {id:'kuaishou',label:'快手推荐',unit:'条'}, {id:'feigua',label:'飞瓜视频库',unit:'页'}];
...
export function PlatformTasks({tasks,selected,resolved,busy,onSelect}: Props) {
  ...
  return <div className="platform-picker" ref={root}>
    <button ref={trigger} className="platform-trigger" aria-label={`平台任务：${currentLabel || '自动跟随'}`} aria-haspopup="dialog" aria-expanded={open} aria-controls="platformTasksPopover" onClick={()=>setOpen(!open)}>
      ...
      <span>{current==='feigua'?'飞瓜':current==='douyin'?'抖音':current==='kuaishou'?'快手':'平台'}</span><span aria-hidden="true">⌄</span>
    </button>
    ...
    <details className="platform-help"><summary>{currentLabel?`当前操作仅作用于${currentLabel}`:'切换到抖音、快手或飞瓜网页以跟随任务'}</summary><p>选择任务只切换控制对象。开启自动跟随后随当前标签页切换；手动选择平台会关闭自动跟随。各平台可同时运行，暂停、继续和结束各自独立。</p></details>
  </section></div>;
}
