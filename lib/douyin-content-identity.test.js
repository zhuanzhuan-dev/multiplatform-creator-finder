...
for (const duration of [120, NaN]) {
  test(`author live entry does not override a video with duration ${duration}`, () => {
    const entry = { ...node({ tag: 'A', text: '进入直播间' }), href: 'https://live.douyin.com/123456', querySelectorAll: () => [] };
    const result = identify(card([node({ tag: 'VIDEO', duration }), entry]));
    assert.equal(result.contentType, 'video');
    assert.equal(contentSkipDecision(result), null);
  });
}

test('a live link wrapping the player is live content', () => {
  const video = node({ tag: 'VIDEO' });
  const entry = { ...node({ tag: 'A' }), href: 'https://live.douyin.com/123456', querySelectorAll: () => [video] };
  assert.equal(identify(card([entry, video])).contentType, 'live');
});
