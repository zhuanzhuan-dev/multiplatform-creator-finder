...
export function createKuaishouRuntime(deps) {
  ...
  async function openPage({tabId,windowId,focus=true,reuseExisting=true}={}){
    ...
    if(!valid(tab)){
      ...
      if(reuseExisting) tab=(await chrome.tabs.query({windowId:win,url:'https://www.kuaishou.com/*'})).find(t=>t.windowId===win && valid(t));
      else tab=null;
  ...
  function start(options={}){if(active())return Promise.reject(Error('快手任务正在运行'));const own=++generation;return serial(async()=>{
    ...
    state={...initial(),status:'starting',runId:crypto.randomUUID(),elapsedMs:0,backgroundMode:scheduled?'scheduled-tab':'current-tab',runTrigger:scheduled?'schedule':'manual',scheduledFor:options.scheduledFor||null,runTarget:normalizeRunTarget(scheduled?options.target:deps.settings().target),runSettings:structuredClone(deps.settings()),runRules:structuredClone(deps.rules()),startupStage:'正在打开快手推荐页'};seen=[];creators=[];
    await save();try{
      const launched=await launch(await openPage({...options,focus:scheduled?false:options.focus!==false,reuseExisting:options.reuseExisting!==false}),own);
      if(scheduled && own===generation){
        const target=state.runTarget;
        const targetText=target.mode==='time'?`${target.durationMinutes} 分钟`:target.mode==='count'?`${target.maxItems} 条视频`:`${target.durationMinutes} 分钟或 ${target.maxItems} 条视频`;
        state.lastScheduleAt=new Date().toISOString();state.lastScheduleResult=`已按计划启动：${targetText}`;await save();
      }
      return launched;
    }catch(e){if(own===generation)await pause(e.message);throw e;}
  });}
  ...
  async function message(message,sender){
    ...
    return serial(async()=>{
      ...
      state.lastTickAt=new Date().toISOString();await save();void deps.scheduleUpload(true);
      if(own!==generation)return {ok:true,continue:false};
  ...
  return {state:()=>state,active,start,noteSchedule,resume,pause,stop,message,inspect,timers,restore,storage:()=>({draKuaishou:{state,seen,creators}}),clear:()=>{state=initial();seen=[];creators=[];}};
}
