...
export const FEIGUA_PAGE_WAIT_MS = 30_000;
export const FEIGUA_ROW_STABLE_MS = 1_500;
export const FEIGUA_IMAGE_GRACE_MS = 8_000;
export const FEIGUA_DETAIL_REQUEST_GAP_MS = 1_000;
...
export function feiguaNextPageWaitMs({ pageStartedAt, lastDetailRequestFinishedAt = 0, now = Date.now(), delayMs }) {
  const pageWait = Math.min(pageStartedAt, now) + delayMs - now;
  const detailAt = Number.isFinite(lastDetailRequestFinishedAt) ? Math.min(lastDetailRequestFinishedAt, now) : 0;
  const detailWait = detailAt > 0 ? detailAt + FEIGUA_DETAIL_REQUEST_GAP_MS - now : 0;
  return Math.max(0, pageWait, detailWait);
}
