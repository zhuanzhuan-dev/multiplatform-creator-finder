...
export async function xingtuObservedCreators(decisions) {
  ...
  return transaction('readonly',(tx,done,on)=>{
    ...
    const range=IDBKeyRange.bound(['xingtu',0],['xingtu',Number.MAX_SAFE_INTEGER]);
    on(tx.objectStore('records').index('sourceSequence').openCursor(range,'prev'),cursor=>{
      if(!cursor)return;
      const item=cursor.value;
      const key=JSON.stringify([item.sessionId,item.raw?.xingtuId]);
      if(item.mode==='browse' && wanted.has(key)) {
        const raw=item.raw;
        const creator={
        ...
        for(const id of wanted.get(key))found[id]=creator;
        wanted.delete(key);
      }
      if(wanted.size)cursor.continue();
    });
  });
}
