...
import { rowAccountKey as xingtuAccountKey } from './xingtu-parser.js';
import { xingtuCommercialFields } from './xingtu-collection.js';
...
function openDatabase() {
  return new Promise((resolve,reject)=>{
    ...
    request.onupgradeneeded=event=>{
      ...
      if(!records.indexNames.contains('expiry'))records.createIndex('expiry',['cleanupAt','id']);
      records.createIndex('creatorLookup',['sourcePlatform','sessionId','entityKey','mode','sequence']);
...
export async function observationStatus(source='feigua') {
  await ensureInitialized();
  return transaction('readonly',(tx,done,on)=>{
    const records=tx.objectStore('records'),result={count:0,latest:[]};done(result);
    on(records.index('source').count(source),count=>{result.count=count;});
    on(records.index('sourceSequence').openCursor(IDBKeyRange.bound([source,0],[source,Number.MAX_SAFE_INTEGER]),'prev'),cursor=>{
      if(cursor){result.latest.push(cursor.value);if(result.latest.length<5)cursor.continue();}
    });
  });
}
export async function xingtuObservedCreators(decisions) {
  const wanted=new Map();
  for(const decision of decisions || []) {
    if(decision?.code!=='XINGTU_OBSERVED' || !/^\d{1,30}$/.test(String(decision.authorId || '')))continue;
    const key=JSON.stringify([decision.runId,decision.authorId]);
    if(!wanted.has(key))wanted.set(key,[]);
    wanted.get(key).push(decision.id);
  }
  if(!wanted.size)return {};
  await ensureInitialized();
  return transaction('readonly',(tx,done,on)=>{
    const found={};done(found);
    const index=tx.objectStore('records').index('creatorLookup');
    for(const [key,ids] of wanted){
      const [sessionId,xingtuId]=JSON.parse(key);
      const prefix=['xingtu',sessionId,`star:${xingtuId}`,'browse'];
      const range=IDBKeyRange.bound([...prefix,0],[...prefix,Number.MAX_SAFE_INTEGER]);
      on(index.openCursor(range,'prev'),cursor=>{
        if(!cursor)return;
        const item=cursor.value;
        const raw=item.raw;
        const creator={
          followers:String(raw.followerCount || ''),
          xingtuIndex:String(raw.xingtuIndex || ''),
          tags:Array.isArray(raw.tags)?raw.tags.filter(tag=>typeof tag==='string').slice(0,20):[],
          prices:raw.prices && typeof raw.prices==='object'?raw.prices:{},
          metrics:raw.metrics && typeof raw.metrics==='object'?raw.metrics:{},
          city:String(raw.city || ''),gender:String(raw.gender || '')
        };
        for(const id of ids)found[id]=creator;
      });
    }
  });
}
...
export async function enableCreatorObservationUpload() {
  await ensureInitialized();
  return transaction('readwrite',(tx,done)=>{
    const count={feigua:0,xingtu:0};
    const cursor=tx.objectStore('records').openCursor();
    cursor.onsuccess=()=>{
      const row=cursor.result;
      if(!row){done(count);return;}
      const item=row.value;
      const stableXingtu=item.sourcePlatform==='xingtu' && /^\d{1,30}$/.test(String(item.raw?.xingtuId || ''));
      const stableFeigua=item.sourcePlatform==='feigua' && Boolean(item.raw?.secUid || item.raw?.uid || item.raw?.xingtuId || item.raw?.bloggerId);
      if(!item.uploadEligible && !item.uploadConfirmed && (stableXingtu || stableFeigua)){
        row.update(storedObservation({...item,uploadEligible:true}));
        count[item.sourcePlatform]++;
      }
      row.continue();
    };
  });
...
export function normalizeXingtuObservation(row,page,sessionId,mode) {
  const identity=row.xingtuId ? `star:${row.xingtuId}` : `account:${xingtuAccountKey(row)}`;
  return { schemaVersion:1, sessionId, mode, sourcePlatform:'xingtu', accountPlatform:'douyin',
    entityKey:identity, viewKey:row.selectedTab || '', surface:page.surface || 'market', observedAt:page.capturedAt || new Date().toISOString(),
    pageUrl:page.pageUrl, authorName:row.authorName || '', avatarUrl:row.avatarUrl || '',
    profileUrl:row.profileUrl || '', followers:parseCompactNumber(row.followerCount),
    videoId:'', caption:'', coverUrl:'', durationSeconds:null, likes:null, comments:null,
    shares:null, favorites:null, plays:null, spreadIndex:null, publishedTimeText:'',
    raw:row };
}
export function xingtuObservationRecord(item) {
  const record=buildObservationRecord({observation:{...item,contentType:'unknown',sourcePlatform:'xingtu',profileUrl:'',beforeUrl:item.pageUrl,hashtags:item.raw.tags},profile:{followers:item.followers},runId:item.sessionId,feedIndex:item.sequence,isRelevant:false,decision:'observed',action:'collect'});
  record.record_id=item.id;
  Object.assign(record.rpa_feedback,{source:item.mode==='automatic'?'xingtu-author-market':'xingtu-browser',collection_mode:item.mode,account_platform:'douyin',xingtu_profile_url:item.profileUrl,xingtu_id:item.raw.xingtuId || '',unique_id:item.raw.uniqueId || '',raw_fields:item.raw,surface:item.surface});
  if (/^\d{1,30}$/.test(String(item.raw.xingtuId || ''))) record.xingtu_id=item.raw.xingtuId;
  Object.assign(record,xingtuCommercialFields(item.raw));
  return record;
}
export function observationRecord(item) {
  const decision=item.raw.ruleDecision;
  const record=buildObservationRecord({observation:{...item,contentType:item.surface==='creator'?'unknown':'video',sourcePlatform:item.sourcePlatform,profileUrl:item.raw.douyinProfileUrl || '',beforeUrl:item.pageUrl,hashtags:item.raw.topics},profile:{followers:item.followers},runId:item.sessionId,feedIndex:item.sequence,isRelevant:Boolean(decision?.matched),decision:decision ? decision.matched ? 'keep' : decision.needsReview ? 'review' : 'skip' : 'observed',action:'collect',extra:decision ? {finderDecision:decision.code,reasons:decision.reasons} : {}});
  if(decision) record.rpa_feedback.review_required=decision.needsReview;
  record.record_id=item.id;
  record.comment_count=item.comments; record.share_count=item.shares; record.favorite_count=item.favorites;
  Object.assign(record.rpa_feedback,{source:item.mode==='automatic'?'feigua-video-library':'feigua-browser',collection_mode:item.mode,account_platform:'douyin',feigua_profile_url:item.profileUrl,feigua_blogger_id:item.raw.bloggerId || '',feigua_blogger_uid:item.raw.feiguaBloggerUid || '',feigua_detail_uid:item.raw.feiguaDetailUid || '',unique_id:item.raw.uniqueId || '',xingtu_profile_url:item.raw.xingtuUrl || '',play_count:item.plays,spread_index:item.spreadIndex,raw_fields:item.raw,surface:item.surface});
  if (item.raw.uid) record.uid=item.raw.uid;
  if (item.raw.secUid) record.sec_uid=item.raw.secUid;
  if (item.raw.xingtuId) record.xingtu_id=item.raw.xingtuId;
  return record;
}
