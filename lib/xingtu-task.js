...
export function createXingtuRuntime(deps) {
  ...
  return {state:()=>state,collection:()=>xingtuCollection,restore,start,resume,stop,pause,message,openPage,inspect:()=>serial(inspect),timers,active,storage:()=>({draXingtuState:state,draXingtuCollection:xingtuCollection}),clear:()=>{xingtuCollection=null;},increment};
async function handleXingtuPage(page, runId, loopId) {
  ...
  if (Object.keys(copy.accounts).length > 900 || copy.seen.length > 5060) throw new Error("星图本轮本地候选已达容量，请结束本轮后再开始");
