...
test('history keeps video metrics and filters by author',()=>{
  ...
  assert.equal(entry.tname,'美食');
  assert.equal(entry.plays,150000);
  assert.equal(historyMatches(entry,{query:'美食'}),true);
  assert.equal(entry.durationSeconds,125);
  assert.equal(historyMatches(entry,{authorId:'12345'}),true);
  assert.equal(historyMatches(entry,{authorId:'999'}),false);
  assert.equal(historyMatches(entry,{profileUrl:'https://space.bilibili.com/12345'}),true);
});
test('missing creator video metrics stay missing while observed zero is retained',()=>{
  const entry=normalizeHistoryEntry({occurredAt:new Date().toISOString(),sourcePlatform:'xingtu',plays:null,likes:'',comments:'  ',favorites:0});
  assert.equal(entry.plays,null);
  assert.equal(entry.likes,null);
  assert.equal(entry.comments,null);
  assert.equal(entry.favorites,0);
});
