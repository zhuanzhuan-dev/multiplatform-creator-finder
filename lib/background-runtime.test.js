...
async function worker(saved = null, fetchImpl = null) {
  ...
  const context = { chrome, crypto:globalThis.crypto, structuredClone, URL, TextEncoder, TextDecoder, AbortController, setTimeout, clearTimeout, console, fetch:fetchImpl || (()=>{throw new Error('Unexpected network call');}) };
...
test('resume preserves the run, results and remaining time across a worker restart', async () => {
  ...
  const saved = {
    ...
    draState: { status: 'paused', runId: 'same-run', loopId: 'old-loop', feedTabId: 7,
      ...
      stats: { scanned: 19, matched: 12 }, recentDecisions: [{ code: 'ACCEPTED', accountName: 'example' }],
  ...
  assert.equal(w.state().recentDecisions.length, 1);
