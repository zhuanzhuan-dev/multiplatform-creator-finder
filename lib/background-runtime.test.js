...
const bundle = await build({
  stdin: { contents: source + '\nglobalThis.runtimeTest = { exportObservations, feigua, xingtu, kuaishou, bilibili, bilibiliPopular, initialize, watchdog, setPaused, stopRun, configureOpenTabSidePanels, flushOutbox, handleUploadAlarm, runCreatorUploadBackfill, scheduleUpload, resumeRun, state: () => state };', resolveDir: root, loader: 'js' },
...
const acceptedFixture=async(_url,options)=>({ok:true,status:200,json:async()=>({accepted:JSON.parse(options.body).records.map(row=>row.record_id)})});
...
test('upgrade moves retained Xingtu creator observations through the shared ingest queue',async()=>{
 ...
 const requests=[];
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{dryRun:false,schedule:{enabled:false}},draXingtuConfig:{settings:{xingtuUploadEnabled:false,dryRun:false,xingtuTarget:{maxPages:50},xingtuDelay:{minSeconds:4,maxSeconds:6}},rules:{xingtuKeywords:[]}},draObservations:{sequence:1,records:[observation]}},async(_url,options)=>{const body=JSON.parse(options.body);requests.push(body);return {ok:true,status:200,json:async()=>({accepted:body.records.map(row=>row.record_id)})};});
...
test('Xingtu browsing sends creator identity and commercial fields after saving each page',async()=>{
 ...
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{dryRun:true,schedule:{enabled:false}}},acceptedFixture);
 w.chrome.tabs.get=async()=>({id:9,url,active:true});
