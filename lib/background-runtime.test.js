...
const bundle = await build({
  stdin: { contents: source + '\nglobalThis.runtimeTest = { exportObservations, feigua, kuaishou, initialize, watchdog, setPaused, stopRun, configureOpenTabSidePanels, flushOutbox, handleUploadAlarm, resumeRun, state: () => state };', resolveDir: root, loader: 'js' },
...
test('fresh installation resolves each platform through manual selection and automatic following',async()=>{
 ...
 w.setTab({url:'https://example.com/'});
 for(const platform of ['douyin','kuaishou','feigua'])assert.equal((await w.message({type:'DRA_GET_STATUS',tabId:7,platform})).state.route?.platform,platform);
