...
const bundle = await build({
  stdin: { contents: source + '\nglobalThis.runtimeTest = { exportObservations, feigua, xingtu, kuaishou, bilibili, bilibiliPopular, initialize, watchdog, setPaused, stopRun, configureOpenTabSidePanels, flushOutbox, handleUploadAlarm, runCreatorUploadBackfill, scheduleUpload, resumeRun, state: () => state, outbox: () => outbox };', resolveDir: root, loader: 'js' },
...
const acceptedFixture=async(url,options)=>({ok:true,status:200,json:async()=>String(url).includes('lookup_xingtu_sec_uid')
  ? {matches:[]} : {accepted:JSON.parse(options.body).records.map(row=>row.record_id)}});
async function worker(saved = null, fetchImpl = null, connected = false, database = new IDBFactory()) {
  const stored = saved || {
  ...
  if(connected)stored.draCloudAuth={device_token:'fixture'};
  ...
  const context = { indexedDB:database,IDBKeyRange,testHistory, chrome, crypto:globalThis.crypto, structuredClone, URL, URLSearchParams, TextEncoder, TextDecoder, AbortController, setTimeout, clearTimeout, console, fetch:fetchImpl || (connected ? acceptedFixture : (()=>{throw new Error('Unexpected network call');})) };
  vm.runInNewContext(bundle.outputFiles[0].text, context);
  await context.runtimeTest.initialize();
  await new Promise(resolve=>setImmediate(resolve));
  return {
    stored, sent, commands, alarms, chrome, database, ...context.runtimeTest,
...
test('upload authorization failure leaves an active collection running',async()=>{
 const w=await worker({
  draCloudAuth:{device_token:'fixture-token'},
  draSettings:{schedule:{enabled:false}},
  draState:{status:'running',runId:'run',feedTabId:7,loopId:'loop'},
  draOutbox:[{id:'pending-record',sessionId:'run',status:'pending',transitionPending:false,queuedAt:new Date().toISOString(),payload:{record_id:'pending-record',feed_index:1}}]
 },async()=>({ok:false,status:401,json:async()=>({error:'revoked'})}));
 const result=await w.flushOutbox({force:true});
 assert.equal(result.status,'auth_error');
 assert.equal(w.state().status,'running');
 assert.equal(w.stored.draCloudAuth.expired,true);
 assert.equal(w.stored.draOutbox[0].status,'pending');
});
...
test('upload latency cannot hold transition confirmation, progress or pause; paused queues restart', async () => {
  ...
  }, async () => { requests++;started();await new Promise(resolve => { release = resolve; });return { ok: true, status: 200, json: async () => ({ accepted: ['record'] }) }; });
...
async function feiguaWorker() {
 const w = await worker({
  draCloudAuth:{device_token:'fixture'},
  draSettings: { dryRun:true, cloud:{enabled:false}, schedule:{enabled:false} },
...
const fgRow = (authorName, title='美食教程') => {
 const bloggerId=Array.from(authorName, character=>character.codePointAt(0)).join('');
 return {authorName,title,videoId:authorName+title,bloggerId,profileUrl:`https://dy.feigua.cn/app/#/blogger-detail/index?bloggerId=${bloggerId}`,topics:[],hotWords:[],followerCount:'12w'};
};
test('Feigua uploads page observations while keeping local candidate decisions through pause', async()=>{
 const w=await feiguaWorker();
 ...
 assert.equal(w.stored.draOutbox.length,0,'page collection waits only for durable local observations');
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox.length,2);
 ...
 await w.stopRun();
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox.length,3);
 assert.equal(w.stored.draOutbox[0].payload.author,'账号甲');
...
test('Feigua keeps paging while upload alarm scheduling is stalled',async()=>{
 const w=await feiguaWorker();
 let release;
 const pendingAlarm=new Promise(resolve=>{release=resolve;});
 const getAlarm=w.chrome.alarms.get;
 w.chrome.alarms.get=name=>name==='dra-upload'?pendingAlarm:getAlarm(name);
 let timeout;
 try {
  const page=w.message({type:'DRA_FEIGUA_PAGE',page:fgPage('independent-upload',[fgRow('继续翻页')])});
  const result=await Promise.race([page,new Promise((_,reject)=>{timeout=setTimeout(()=>reject(Error('采集等待了上传调度')),1000);})]);
  assert.equal(result.continue,true);
  assert.equal(w.stored.draFeiguaState.stats.scanned,1);
  assert.equal(w.stored.draOutbox.length,0);
 } finally {clearTimeout(timeout);release(null);w.chrome.alarms.get=getAlarm;}
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox.length,1);
});
test('Feigua full-page exclusion precedes page rest and preserves complete page', async()=>{
 ...
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox.length,3);assert.equal(w.stored.draFeiguaCollection.finalized,false);
});
...
test('Feigua queue survives a service worker restart before finalization', async()=>{
 ...
 const restarted=await worker(structuredClone(w.stored),null,false,w.database);
 await restarted.scheduleUpload();
...
async function parallelWorker() {
 const w=await worker(null,null,true);
...
test('a full upload queue does not pause Douyin collection or discard pending records',async()=>{
 ...
 assert.equal(result.continue,true);assert.equal(w.state().status,'running');
 assert.equal(w.stored.draOutbox.length,1001);assert.equal(w.stored.draOutbox[0].id,'queued-0');
 assert.equal(w.stored.draSeenVideos.includes('not-lost'),true);
});
...
test('Feigua keeps complete observations locally while the shared upload queue is full',async()=>{
 const pending=Array.from({length:1000},(_,i)=>({id:`full-${i}`,sessionId:'previous',status:'pending',payload:{record_id:`full-${i}`,feed_index:i+1}}));
 const w=await worker({draSettings:{dryRun:false,schedule:{enabled:false}},draFeiguaState:{status:'running',runId:'fg-full',loopId:'loop',feedTabId:7,feedWindowId:1,startedAt:new Date().toISOString()},draFeiguaCollection:{runId:'fg-full',dryRun:false,accounts:{},excluded:{},seen:[],pages:[],finalized:false,keywords:[]},draOutbox:pending});
 w.setTab({url:feiguaUrl});
 const result=await w.message({type:'DRA_FEIGUA_PAGE',platform:'feigua',page:fgPage('p',[fgRow('候选保留')])});
 assert.notEqual(result.ok,false);
 assert.equal(w.stored.draOutbox.length,1000);
 assert.equal((await w.exportObservations()).some(item=>item.authorName==='候选保留' && item.uploadEligible),true);
 assert.equal(Object.keys(w.stored.draFeiguaCollection.accounts).length,1);
});
...
test('Feigua formal uploads share the queue even with a persisted legacy off switch',async()=>{
 const sent=[];
 ...
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{feiguaUploadEnabled:false,schedule:{enabled:false}},draState:{runId:'dy'},draOutbox:[item('fg','feigua'),item('dy','douyin')]},async(_,options)=>{const ids=JSON.parse(options.body).records.map(r=>r.record_id);sent.push(...ids);return {ok:true,status:200,json:async()=>({accepted:ids})};});
 await w.flushOutbox({force:true});
 await w.flushOutbox({force:true});
 assert.deepEqual(sent,['fg','dy']);
 assert.equal(w.stored.draOutbox.find(item=>item.id==='fg').status,'written');
 assert.equal(w.stored.draSettings.feiguaUploadEnabled,undefined);
});
test('restoring a Feigua collection resumes automatic upload despite old local test mode',async()=>{
 const w=await worker({draSettings:{dryRun:true,schedule:{enabled:false}},draFeiguaState:{status:'paused',runId:'fg'},draFeiguaCollection:{runId:'fg',dryRun:false,accounts:{one:{authorName:'本地飞瓜',feedIndex:1}},excluded:{},seen:[],pages:[],finalized:false}});
 assert.equal(w.feigua.state().runSettings.dryRun,false);
 await w.feigua.stop();
 assert.equal(w.stored.draOutbox[0].status,'pending');
});
...
test('Feigua next-run settings can be changed while Douyin runs and do not alter its target',async()=>{
 ...
 const response=await new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0]({type:'DRA_SAVE_FEIGUA_CONFIG',positiveKeywords:['摄影'],maxPages:50,minSeconds:4,maxSeconds:6,keywords:['恐怖','测试词']},{url:'chrome-extension://test/sidepanel/index.html'},resolve));
 assert.equal(response.ok,true,response.error);
 ...
 assert.deepEqual(w.stored.draRules.feiguaPositiveKeywords,['摄影']);
});
...
test('Feigua rejects invalid page and delay settings without saving',async()=>{
 ...
 for(const patch of [{maxPages:0},{maxPages:1.5},{minSeconds:0},{maxSeconds:31},{minSeconds:7,maxSeconds:6}]) {
  const result=await new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0]({type:'DRA_SAVE_FEIGUA_CONFIG',positiveKeywords:['摄影'],maxPages:50,minSeconds:4,maxSeconds:6,keywords:[],...patch},{url:'chrome-extension://test/sidepanel/index.html'},resolve));
...
test('editing paused Kuaishou settings does not require pausing running Douyin',async()=>{
 const w=await worker();const original=structuredClone(w.stored.draSettings);
 const ks=await w.message({type:'DRA_GET_STATUS',platform:'kuaishou'});
 const saved=await w.message({type:'DRA_SAVE_CONFIG',platform:'kuaishou',settings:{...ks.settings,target:{mode:'count',maxItems:17,durationMinutes:60}}});
 assert.equal(saved.ok,true,saved.error);assert.deepEqual(w.stored.draSettings,original);
 assert.equal((await w.message({type:'DRA_GET_STATUS',platform:'kuaishou'})).settings.target.maxItems,17);
 assert.equal(w.state().status,'running');
});
test('Kuaishou draft and rule edits remain isolated across worker restart',async()=>{
 const w=await worker();await w.setPaused('edit');
 const dy=await w.message({type:'DRA_GET_STATUS',platform:'douyin'});
 await w.message({type:'DRA_SAVE_CONFIG',platform:'kuaishou',saveDraft:true,settings:{...dy.settings,target:{mode:'count',maxItems:'',durationMinutes:60}}});
 assert.equal((await w.message({type:'DRA_GET_STATUS',platform:'douyin'})).settingsDraft,null);
 const newRules={...dy.rules,hard:{...dy.rules.hard,minVideoLikes:12345}};
 assert.equal((await w.message({type:'DRA_SAVE_CONFIG',platform:'kuaishou',rules:newRules})).ok,true);
 const next=await worker(structuredClone(w.stored));
 assert.equal((await next.message({type:'DRA_GET_STATUS',platform:'kuaishou'})).rules.hard.minVideoLikes,12345);
 assert.equal((await next.message({type:'DRA_GET_STATUS',platform:'douyin'})).rules.hard.minVideoLikes,dy.rules.hard.minVideoLikes);
 assert.ok((await next.message({type:'DRA_GET_STATUS',platform:'kuaishou'})).settingsDraft);
});


test('Douyin and Kuaishou can run together and pause independently',async()=>{
 const w=await worker();
 const dyTab={id:7,windowId:1,status:'complete',url:'https://www.douyin.com/?recommend=1'};
 const ksTab={id:8,windowId:1,status:'complete',url:'https://www.kuaishou.com/new-reco'};
 w.chrome.tabs.get=async id=>id===8?ksTab:dyTab;
 w.chrome.tabs.query=async()=>[dyTab,ksTab];
 await w.kuaishou.start({tabId:8,windowId:1});
 assert.equal(w.kuaishou.state().status,'running');assert.equal(w.state().status,'running');
 const locked=await w.message({type:'DRA_SAVE_CONFIG',platform:'kuaishou',settings:{...w.stored.draKuaishouConfig.settings,target:{mode:'count',maxItems:99,durationMinutes:60}}});
 assert.equal(locked.ok,false);
 await w.kuaishou.pause();assert.equal(w.state().status,'running');
 await w.kuaishou.resume({windowId:1});await w.setPaused('user');
 assert.equal(w.kuaishou.state().status,'running');
});

test('Kuaishou shares serial uploads with Douyin and receives its own counts',async()=>{
 const item=(id,sessionId)=>({id,sessionId,status:'pending',queuedAt:new Date().toISOString(),payload:{record_id:id,feed_index:1}});
 const batches=[];
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{schedule:{enabled:false}},draState:{runId:'dy'},draKuaishou:{state:{runId:'ks',status:'stopped'}},draOutbox:[item('d1','dy'),item('k1','ks'),item('d2','dy')]},async(_,options)=>{
  const ids=JSON.parse(options.body).records.map(row=>row.record_id);batches.push(ids);return {ok:true,status:200,json:async()=>({accepted:ids})};
 });
 await Promise.all([w.flushOutbox({force:true,limit:1}),w.flushOutbox({force:true,limit:1})]);
 await w.flushOutbox({force:true,limit:1});await w.flushOutbox({force:true,limit:1});
 assert.deepEqual(batches,[['d1'],['k1'],['d2']]);
 assert.equal(w.state().stats.uploaded,2);assert.equal(w.kuaishou.state().stats.uploaded,1);
});

test('launcher selection opens immediately and selects only the requested window platform',async()=>{
 const w=await worker();const opened=[];
 w.chrome.sidePanel.open=async options=>opened.push(options.windowId);
 const dispatch=(message,sender)=>new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0](message,sender,resolve));
 const openedResult=await dispatch({type:'DRA_OPEN_PANEL',platform:'kuaishou'},{tab:{id:7,windowId:1},url:'https://www.douyin.com/',frameId:0});
 assert.equal(openedResult.ok,true);assert.deepEqual(opened,[1]);
 const sender={url:'chrome-extension://test/sidepanel/index.html'};
 assert.equal((await dispatch({type:'DRA_GET_PANEL_SELECTION',windowId:2},sender)).platform,null);
 assert.equal((await dispatch({type:'DRA_GET_PANEL_SELECTION',windowId:1},sender)).platform,'kuaishou');
 assert.equal((await dispatch({type:'DRA_GET_PANEL_SELECTION',windowId:1},sender)).platform,null);
});

test('fresh installation shows the Douyin task without requiring a previous run',async()=>{
 const w=await worker({});
 for(const message of [{type:'DRA_GET_STATUS',platform:'douyin'},{type:'DRA_GET_STATUS',platform:'auto',tabId:7}]){
  const response=await w.message(message);
  assert.equal(response.state.route?.platform,'douyin');
  assert.equal(response.state.route?.surface,'recommend');
  assert.equal(response.state.status,'idle');
  assert.equal(response.state.runId,'');
  assert.equal(response.state.feedTabId,null);
 }
 w.setTab({url:'https://example.com/'});
 const automatic=await w.message({type:'DRA_GET_STATUS',platform:'auto',tabId:7});
 assert.equal(automatic.state.route,null);
 const manual=await w.message({type:'DRA_GET_STATUS',platform:'douyin',tabId:7});
 assert.equal(manual.state.route?.platform,'douyin');
});

test('fresh installation resolves each platform through manual selection and automatic following',async()=>{
 const w=await worker({});
 for(const [platform,url] of [['douyin','https://www.douyin.com/?recommend=1'],['kuaishou','https://www.kuaishou.com/new-reco'],['feigua','https://dy.feigua.cn/app/#/video/library/all']]){
  w.setTab({url});
  for(const selected of ['auto',platform]){
   const status=await w.message({type:'DRA_GET_STATUS',tabId:7,platform:selected});
   assert.equal(status.state.route?.platform,platform);
   assert.equal(status.configPlatform,platform);
   assert.equal(status.state.status,'idle');
   assert.equal(status.decisionHistory.length,0);
  }
 }
 for(const url of ['https://dy.feigua.cn/app/#/video-detail/index?awemeId=123','https://dy.feigua.cn/app/#/blogger-detail/index?bloggerId=123']){
  w.setTab({url});const status=await w.message({type:'DRA_GET_STATUS',tabId:7,platform:'auto'});
  assert.equal(status.state.route.platform,'feigua');assert.equal(status.browsing.enabled,true);assert.equal(status.settings.feiguaUploadEnabled,undefined);
 }
 w.setTab({url:'https://example.com/'});
 for(const platform of ['douyin','kuaishou','feigua','xingtu'])assert.equal((await w.message({type:'DRA_GET_STATUS',tabId:7,platform})).state.route?.platform,platform);
 assert.equal((await w.message({type:'DRA_GET_STATUS',tabId:7,platform:'auto'})).state.route,null);
});
test('restored empty platform routes do not hide Feigua or Kuaishou task pages',async()=>{
 const w=await worker({draFeiguaState:{route:null,status:'idle'},draKuaishou:{state:{route:null,status:'idle'}}});
 for(const platform of ['feigua','kuaishou'])assert.equal((await w.message({type:'DRA_GET_STATUS',platform})).state.route?.platform,platform);
});
test('Xingtu settings stay isolated from Douyin targets and video rules',async()=>{
 const w=await worker({
  draSettings:{target:{mode:'both',durationMinutes:90,maxItems:12},schedule:{enabled:false}},
  draRules:{hard:{minVideoLikes:3000},xingtuKeywords:['旧词']},
  draXingtuConfig:{settings:{xingtuTarget:{maxPages:7},xingtuDelay:{minSeconds:4,maxSeconds:6},dryRun:true},rules:{xingtuKeywords:['游戏']}}
 });
 const xingtu=await w.message({type:'DRA_GET_STATUS',platform:'xingtu'});
 assert.equal(xingtu.settings.xingtuTarget.maxPages,7);
 assert.equal(xingtu.settings.target,undefined);
 assert.equal(xingtu.rules.xingtuKeywords.join(','),'游戏');
 assert.equal(xingtu.rules.hard,undefined);
 const douyin=await w.message({type:'DRA_GET_STATUS',platform:'douyin'});
 assert.equal(douyin.settings.target.maxItems,12);
 assert.equal(douyin.rules.hard.minVideoLikes,3000);
});

test('upgrade moves retained Xingtu creator observations through the shared ingest queue',async()=>{
 const observedAt=new Date().toISOString();
 const observation={id:'xingtu-local-record',sequence:1,sessionId:'xingtu-run',mode:'automatic',sourcePlatform:'xingtu',accountPlatform:'douyin',surface:'market',entityKey:'star:987654',viewKey:'',observedAt,firstObservedAt:observedAt,lastObservedAt:observedAt,pageUrl:'https://www.xingtu.cn/ad/creator/market',authorName:'星图达人',followers:126000,raw:{xingtuId:'987654',authorName:'星图达人',followerCount:'12.6w',metrics:{'预期CPM':'23.8'},tags:['美食']},uploadEligible:false};
 const requests=[];
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{dryRun:false,schedule:{enabled:false}},draXingtuConfig:{settings:{xingtuUploadEnabled:false,dryRun:false,xingtuTarget:{maxPages:50},xingtuDelay:{minSeconds:4,maxSeconds:6}},rules:{xingtuKeywords:[]}},draObservations:{sequence:1,records:[observation]}},async(url,options)=>{if(String(url).includes('lookup_xingtu_sec_uid'))return {ok:true,status:200,json:async()=>({matches:[]})};const body=JSON.parse(options.body);requests.push(body);return {ok:true,status:200,json:async()=>({accepted:body.records.map(row=>row.record_id)})};});
 assert.equal((await w.message({type:'DRA_GET_STATUS'})).ok,true);
 assert.ok(w.alarms.has('dra-creator-upload-backfill'));
 assert.equal(w.stored.draOutbox.length,0,'opening the panel must not wait for the legacy backfill');
 await w.runCreatorUploadBackfill();
 assert.equal(w.stored.draCreatorUploadBackfillV1,true);
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox.length,1);
 assert.equal(w.stored.draOutbox[0].payload.xingtu_id,'987654');
 assert.equal(w.stored.draOutbox[0].payload.rpa_feedback.raw_fields.metrics['预期CPM'],'23.8');
 await w.flushOutbox({force:true});
 assert.equal(w.stored.draOutbox[0].status,'written');
 assert.equal(requests.some(body=>body.records.some(row=>row.xingtu_id==='987654')),true);
});

test('Xingtu browsing sends creator identity and commercial fields after saving each page',async()=>{
 const url='https://www.xingtu.cn/ad/creator/market';
 const calls=[];
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{dryRun:true,schedule:{enabled:false}}},async(endpoint,options)=>{
   calls.push(String(endpoint));
   if(String(endpoint).includes('lookup_xingtu_sec_uid')){
     assert.equal(options.headers.authorization,'Bearer fixture');
     assert.deepEqual(JSON.parse(options.body).xingtu_ids,['987654321']);
     return {ok:true,status:200,json:async()=>({matches:[{xingtu_id:'987654321',sec_uid:'MS4wLjABAAAAknown'}]})};
   }
   return acceptedFixture(endpoint,options);
 });
 w.chrome.tabs.get=async()=>({id:9,url,active:true});
 const page={surface:'market',pageUrl:url,capturedAt:new Date().toISOString(),rows:[{authorName:'星图实采',xingtuId:'987654321',followerCount:'12.6w',prices:{'21-60s':'131,000'},metrics:{'预期CPM':'23.8'},tags:['旅行']}]};
 const response=await new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0]({type:'DRA_XINGTU_BROWSE_PAGE',page},{tab:{id:9,url},url,frameId:0},resolve));
 assert.equal(response.ok,true,response.error);
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox.length,1);
 assert.equal(w.stored.draOutbox[0].payload.xingtu_id,'987654321');
 assert.equal(w.stored.draOutbox[0].payload.quote_21_60s,131000);
 assert.equal(w.stored.draOutbox[0].payload.cpm,23.8);
 assert.ok(w.alarms.has('dra-upload'));
 await w.flushOutbox({force:true});
 assert.equal(w.stored.draOutbox[0].status,'written');
 assert.equal(w.stored.draOutbox[0].payload.sec_uid,'MS4wLjABAAAAknown');
 assert.equal(w.stored.draOutbox[0].payload.rpa_feedback.sec_uid_source,'creator_database_xingtu_id');
 assert.equal(w.stored.draOutbox[0].payload.rpa_feedback.raw_fields.secUid,undefined,'database identity must not be written into Xingtu raw fields');
 assert.equal(calls.filter(endpoint=>endpoint.includes('lookup_xingtu_sec_uid')).length,1);
});

test('failed Xingtu identity lookup retains upload for retry',async()=>{
 const url='https://www.xingtu.cn/ad/creator/market';
 let lookupAttempts=0,ingestAttempts=0;
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{dryRun:true,schedule:{enabled:false}}},async(endpoint,options)=>{
   if(String(endpoint).includes('lookup_xingtu_sec_uid')){
     lookupAttempts++;
     return lookupAttempts===1 ? {ok:false,status:503,json:async()=>({error:'database_read_failed'})}
       : {ok:true,status:200,json:async()=>({matches:[{xingtu_id:'12345',sec_uid:'MS4wLjABAAAAretry'}]})};
   }
   ingestAttempts++;
   return acceptedFixture(endpoint,options);
 });
 w.chrome.tabs.get=async()=>({id:9,url,active:true});
 const page={surface:'market',pageUrl:url,capturedAt:new Date().toISOString(),rows:[{authorName:'待重试达人',xingtuId:'12345'}]};
 const response=await new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0]({type:'DRA_XINGTU_BROWSE_PAGE',page},{tab:{id:9,url},url,frameId:0},resolve));
 assert.equal(response.ok,true,response.error);
 await w.scheduleUpload();
 assert.equal((await w.flushOutbox({force:true})).status,'identity_lookup_error');
 assert.equal(w.stored.draOutbox[0].status,'write-error');
 assert.equal(ingestAttempts,0,'must not upload without the available database identity');
 w.outbox()[0].retryAt=0;
 await w.flushOutbox({force:true});
 assert.equal(w.stored.draOutbox[0].status,'written');
 assert.equal(w.stored.draOutbox[0].payload.sec_uid,'MS4wLjABAAAAretry');
 assert.equal(ingestAttempts,1);
});

test('Xingtu next batch uses the saved page limit after an earlier two-page batch',async()=>{
 const w=await worker({
  draCloudAuth:{device_token:'fixture'},
  draSettings:{dryRun:true,cloud:{enabled:false},schedule:{enabled:false}},
  draXingtuConfig:{settings:{xingtuTarget:{maxPages:50},xingtuDelay:{minSeconds:4,maxSeconds:6},dryRun:true},rules:{xingtuKeywords:[]}},
  draXingtuState:{status:'paused',route:{platform:'xingtu',surface:'market'},runId:'old',loopId:'old-loop',feedTabId:7,feedWindowId:1,startedAt:new Date().toISOString(),runTarget:{mode:'pages',maxPages:2},batchStartPages:2,restUntil:Date.now()+10000,runSettings:{dryRun:true},stats:{pagesCollected:4}},
  draXingtuCollection:{runId:'old',accounts:{},excluded:{},seen:[],pages:['1','2','3','4'],finalized:false}
 },acceptedFixture);
 w.setTab({url:'https://www.xingtu.cn/ad/creator/market'});
 assert.equal(w.xingtu.state().runTarget.maxPages,2);
 await w.xingtu.resume({windowId:1});
 assert.equal(w.xingtu.state().batchStartPages,4);
 assert.equal(w.xingtu.state().runTarget.maxPages,50);
 assert.equal(w.xingtu.state().restUntil,null);
 assert.equal(w.xingtu.state().status,'running');
});

test('Bilibili recommend and popular expose independent idle tasks',async()=>{
 const w=await worker({});
 w.setTab({url:'https://www.bilibili.com/'});
 const recommend=await w.message({type:'DRA_GET_STATUS',tabId:7,platform:'auto'});
 assert.equal(recommend.state.route.surface,'recommend');
 assert.equal(recommend.configPlatform,'bilibili');
 w.setTab({url:'https://www.bilibili.com/v/popular/all'});
 const popular=await w.message({type:'DRA_GET_STATUS',tabId:7,platform:'auto'});
 assert.equal(popular.state.route.surface,'popular');
 assert.equal(popular.configPlatform,'bilibili-popular');
 w.setTab({url:'https://example.com/'});
 const manualPopular=await w.message({type:'DRA_GET_STATUS',tabId:7,platform:'bilibili-popular'});
 assert.equal(manualPopular.state.route.surface,'popular');
 assert.equal(manualPopular.configPlatform,'bilibili-popular');
 const tasks=manualPopular.tasks.filter(task=>task.route?.platform==='bilibili');
 assert.equal(tasks.length,2);
});

test('Bilibili recommend and popular keep separate targets and can run together',async()=>{
 const w=await worker();
 const recTab={id:9,windowId:1,status:'complete',url:'https://www.bilibili.com/'};
 const popTab={id:10,windowId:1,status:'complete',url:'https://www.bilibili.com/v/popular/all'};
 w.chrome.tabs.get=async id=>id===10?popTab:id===9?recTab:{id:7,windowId:1,status:'complete',url:'https://www.douyin.com/?recommend=1'};
 w.chrome.tabs.query=async()=>[recTab,popTab];
 const recommend=await w.message({type:'DRA_GET_STATUS',platform:'bilibili'});
 const popular=await w.message({type:'DRA_GET_STATUS',platform:'bilibili-popular'});
 const savedRecommend=await w.message({type:'DRA_SAVE_CONFIG',platform:'bilibili',settings:{...recommend.settings,target:{mode:'count',maxItems:21,durationMinutes:60}}});
 const savedPopular=await w.message({type:'DRA_SAVE_CONFIG',platform:'bilibili-popular',settings:{...popular.settings,target:{mode:'count',maxItems:44,durationMinutes:60}}});
 assert.equal(savedRecommend.ok,true,savedRecommend.error);
 assert.equal(savedPopular.ok,true,savedPopular.error);
 assert.equal((await w.message({type:'DRA_GET_STATUS',platform:'bilibili'})).settings.target.maxItems,21);
 assert.equal((await w.message({type:'DRA_GET_STATUS',platform:'bilibili-popular'})).settings.target.maxItems,44);
 await w.bilibili.start({tabId:9,windowId:1});
 assert.equal(w.bilibili.state().status,'running');
 assert.equal(w.bilibili.state().runTarget.maxItems,21);
 const popularWhileRecommendRuns=await w.message({type:'DRA_SAVE_CONFIG',platform:'bilibili-popular',settings:{...popular.settings,target:{mode:'count',maxItems:48,durationMinutes:60}}});
 assert.equal(popularWhileRecommendRuns.ok,true,popularWhileRecommendRuns.error);
 await w.bilibiliPopular.start({tabId:10,windowId:1});
 assert.equal(w.bilibiliPopular.state().status,'running');
 assert.equal(w.bilibiliPopular.state().runTarget.maxItems,48);
 const locked=await w.message({type:'DRA_SAVE_CONFIG',platform:'bilibili',settings:{...recommend.settings,target:{mode:'count',maxItems:99,durationMinutes:60}}});
 assert.equal(locked.ok,false);
 await w.bilibiliPopular.pause();
 const popularAfterPause=await w.message({type:'DRA_SAVE_CONFIG',platform:'bilibili-popular',settings:{...popular.settings,target:{mode:'count',maxItems:55,durationMinutes:60}}});
 assert.equal(popularAfterPause.ok,true,popularAfterPause.error);
 assert.equal(w.bilibili.state().status,'running');
 assert.equal((await w.message({type:'DRA_GET_STATUS',platform:'bilibili-popular'})).settings.target.maxItems,55);
});

test('legacy popular Bilibili run restores onto the popular task only',async()=>{
 const w=await worker({
  draSettings:{dryRun:true,cloud:{enabled:false},schedule:{enabled:false}},
  draBilibili:{state:{status:'paused',runId:'legacy-pop',route:{platform:'bilibili',surface:'popular'},stats:{scanned:18},runSettings:{dryRun:true},runTarget:{mode:'count',maxItems:40,durationMinutes:60}},seen:['BV1'],creators:['123']},
  draBilibiliConfig:{settings:{dryRun:true,target:{mode:'count',maxItems:40,durationMinutes:60},schedule:{enabled:false}},rules:{},settingsDraft:null}
 });
 const popular=await w.message({type:'DRA_GET_STATUS',platform:'bilibili-popular'});
 const recommend=await w.message({type:'DRA_GET_STATUS',platform:'bilibili'});
 assert.equal(popular.state.runId,'legacy-pop');
 assert.equal(popular.state.status,'paused');
 assert.equal(popular.state.stats.scanned,18);
 assert.equal(recommend.state.runId,'');
 assert.equal(recommend.state.status,'idle');
 assert.equal(popular.settings.target.maxItems,40);
});

test('Feigua saves a complete page beyond 900 candidates and can finish beyond the preview queue size',async()=>{
 const w=await feiguaWorker();
 for(let i=0;i<999;i++)w.feigua.collection().accounts[`old:${i}`]={...fgRow(`旧账号${i}`),feedIndex:i+1};
 const result=await w.message({type:'DRA_FEIGUA_PAGE',page:fgPage('capacity',Array.from({length:20},(_,i)=>fgRow(`新账号${i}`)))});
 assert.equal(result.continue,true);
 assert.equal(Object.keys(w.feigua.collection().accounts).length,1019);
 assert.equal((await w.exportObservations()).length,20);
 await w.feigua.stop();
 assert.equal(w.feigua.state().status,'stopped');
 assert.ok(w.stored.draOutbox.length<=1000);
});
test('Feigua continues saving complete pages beyond 5000 observations',async()=>{
 const w=await feiguaWorker();
 w.feigua.collection().seen=Array.from({length:4990},(_,i)=>`old:${i}`);
 const result=await w.message({type:'DRA_FEIGUA_PAGE',page:fgPage('boundary',Array.from({length:20},(_,i)=>fgRow(`边界账号${i}`)))});
 assert.equal(result.continue,true);
 assert.equal(w.feigua.state().status,'running');
 assert.equal(w.feigua.collection().seen.length,5010);
 assert.equal((await w.exportObservations()).length,20);
 const next=await w.message({type:'DRA_FEIGUA_PAGE',page:fgPage('after-boundary',[fgRow('后续账号')])});
 assert.equal(next.continue,true);
 assert.equal(w.feigua.collection().seen.length,5011);
 assert.equal((await w.exportObservations()).length,21);
});
for (const mode of ['browse','automatic']) test(`Feigua ${mode} capture schedules upload before the run ends`,async()=>{
 const w=await worker({draCloudAuth:{device_token:'fixture'},draSettings:{feiguaUploadEnabled:false,schedule:{enabled:false}},draFeiguaState:{status:'running',feedTabId:7,runId:'fg',loopId:'loop',runSettings:{dryRun:false},runRules:{feiguaKeywords:[]}},draFeiguaCollection:{runId:'fg',dryRun:false,accounts:{},excluded:{},seen:[],pages:[],finalized:false,keywords:[]}});
 const id=mode==='browse'?9:7;
 w.chrome.tabs.get=async()=>({id,url:feiguaUrl,active:true});
 const page={...fgPage('upload',[{...fgRow('上传账号'),videoId:'123456789',columns:[{label:'新增列',value:'保留'}]}]),surface:'library'};
 const msg=mode==='browse'?{type:'DRA_FEIGUA_BROWSE_PAGE',page}:{type:'DRA_FEIGUA_PAGE',page,runId:'fg',loopId:'loop'};
 const result=await new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0](msg,{tab:{id,url:feiguaUrl},url:feiguaUrl,frameId:0},resolve));
 assert.notEqual(result.ok,false,JSON.stringify(result));
 await w.scheduleUpload();
 assert.equal(w.stored.draOutbox[0].status,'pending');
 assert.equal(w.stored.draOutbox[0].payload.rpa_feedback.raw_fields.columns[0].value,'保留');
 assert.ok(w.alarms.has('dra-upload'));
 assert.equal(w.feigua.state().status,'running');
});

test('Feigua read timeout records a diagnostic without counting or uploading an uncollected page',async()=>{
 const w=await feiguaWorker();
 const result=await w.message({type:'DRA_FEIGUA_PAGE_TIMEOUT',pageUrl:feiguaUrl,pageNumber:'2'});
 assert.equal(result.ok,true);
 assert.equal(w.feigua.state().status,'running');
 assert.equal(w.feigua.state().stats.pageReadTimeouts,1);
 assert.equal(w.feigua.state().stats.pagesCollected || 0,0);
 assert.equal((await w.exportObservations()).length,0);
 assert.equal(w.stored.draOutbox.length,0);
 assert.equal(w.feigua.state().lastDecision.code,'FEIGUA_PAGE_TIMEOUT');
 assert.match(w.feigua.state().lastDecision.reasons[0],/30 秒/);
 const stale=await w.message({type:'DRA_FEIGUA_PAGE_TIMEOUT',pageUrl:feiguaUrl,loopId:'old'});
 assert.equal(stale.ok,false);
 assert.equal(w.feigua.state().stats.pageReadTimeouts,1);
 await w.message({type:'DRA_FEIGUA_COMPLETE'});
 assert.equal(w.feigua.state().status,'stopped');
 assert.match(w.feigua.state().pauseReason,/读取超时/);
});

 test('Feigua runtime records positive, review and exclusion using frozen run keywords', async()=>{
  const w=await feiguaWorker();
  w.feigua.collection().positiveKeywords=['美食'];
  await w.message({type:'DRA_FEIGUA_PAGE',page:fgPage('rules',[fgRow('甲'),fgRow('乙','普通'),fgRow('丙','恐怖美食')])});
  assert.equal(w.state().stats.ruleMatched,1);
  assert.equal(w.state().stats.reviewQueued,1);
  assert.equal(w.state().stats.rejectedProfiles,1);
  await w.message({type:'DRA_FEIGUA_PAGE',page:fgPage('veto',[fgRow('甲','恐怖')])});
  assert.equal(w.state().stats.ruleMatched,0);
  assert.equal(w.state().stats.rejectedProfiles,2);
 });

const extensionMessage = (w, message) => new Promise(resolve => w.chrome.runtime.onMessage.listeners[0](message, {url:'chrome-extension://test/sidepanel/index.html'}, resolve));
const saveFeiguaTestRules = (w, positiveKeywords, keywords) => extensionMessage(w, {type:'DRA_SAVE_FEIGUA_CONFIG',maxPages:50,minSeconds:4,maxSeconds:6,positiveKeywords,keywords});

test('Feigua saved rules drive a new run, survive restart and remain frozen on resume', async()=>{
 const w=await worker(null,null,true);
 w.setTab({url:feiguaUrl});w.setStatus({url:feiguaUrl});
 const saved=await saveFeiguaTestRules(w,[' 摄影 ','摄影'],['喝酒']);
 assert.equal(saved.ok,true,JSON.stringify(saved));
 assert.deepEqual(w.stored.draRules.feiguaPositiveKeywords,['摄影']);
 await w.feigua.start(7);
 const send=page=>w.message({type:'DRA_FEIGUA_PAGE',platform:'feigua',runId:w.feigua.state().runId,loopId:w.feigua.state().loopId,page});
 const page=fgPage('classify',[
  fgRow('正向账号','摄影教程'),
  {...fgRow('小林','普通标题'),hotWords:['喝酒']},
  fgRow('爱喝酒的梅梅','摄影教程'),
  fgRow('未知账号','普通标题')
 ]);
 assert.notEqual((await send(page)).ok,false);
 assert.equal(w.feigua.state().stats.ruleMatched,1);
 assert.equal(w.feigua.state().stats.rejectedProfiles,2);
 assert.equal(w.feigua.state().stats.reviewQueued,1);
 const observations=await w.exportObservations();
 assert.equal(observations.length,4);
 assert.deepEqual(Array.from(observations,item=>item.raw.ruleDecision.code),['FEIGUA_MATCHED','FEIGUA_EXCLUDED','FEIGUA_EXCLUDED','FEIGUA_PENDING_REVIEW']);
 await w.feigua.pause();
 assert.equal((await saveFeiguaTestRules(w,['旅行'],['数码'])).ok,true);
 const restarted=await worker(structuredClone(w.stored),acceptedFixture);
 assert.deepEqual(Array.from(restarted.feigua.collection().positiveKeywords),['摄影']);
 assert.deepEqual(Array.from(restarted.feigua.collection().keywords),['喝酒']);
 w.setStatus({runId:w.feigua.state().runId});
 await w.feigua.resume();
 await send(fgPage('after-save',[fgRow('续跑账号','摄影') ]));
 assert.equal(w.feigua.state().stats.ruleMatched,2);
 await send(fgPage('later-veto',[fgRow('正向账号','喝酒') ]));
 assert.equal(w.feigua.state().stats.ruleMatched,1);
 assert.equal(w.feigua.state().stats.rejectedProfiles,3);
 await w.feigua.pause();
 await w.feigua.start(7);
 assert.deepEqual(Array.from(w.feigua.collection().positiveKeywords),['旅行']);
 await send(fgPage('new-rules',[fgRow('新旅行账号','旅行'),fgRow('新摄影账号','摄影')]));
 assert.equal(w.feigua.state().stats.ruleMatched,1);
 assert.equal(w.feigua.state().stats.reviewQueued,1);
});

test('Feigua browse uploads despite the unrelated local test setting',async()=>{
 const w=await worker({draSettings:{dryRun:true,feiguaUploadEnabled:true,schedule:{enabled:false}}});
 w.chrome.tabs.get=async()=>({id:9,url:feiguaUrl,active:true});
 assert.equal((await saveFeiguaTestRules(w,['摄影'],['喝酒'])).ok,true);
 const page={...fgPage('browse-rules',[
  fgRow('正向浏览','摄影教程'),
  {...fgRow('热词浏览','普通'),hotWords:['喝酒']},
  fgRow('爱喝酒的梅梅','摄影教程'),
  fgRow('待确认浏览','普通')
 ]),surface:'library'};
 const send=()=>new Promise(resolve=>w.chrome.runtime.onMessage.listeners[0]({type:'DRA_FEIGUA_BROWSE_PAGE',page},{tab:{id:9,url:feiguaUrl},url:feiguaUrl,frameId:0},resolve));
 assert.equal((await send()).ok,true);
 await w.scheduleUpload();
 const expected=['FEIGUA_MATCHED','FEIGUA_EXCLUDED','FEIGUA_EXCLUDED','FEIGUA_PENDING_REVIEW'];
 assert.deepEqual(Array.from(await w.exportObservations(),item=>item.raw.ruleDecision.code),expected);
 assert.deepEqual(Array.from(w.stored.__history,item=>item.code),expected);
 assert.deepEqual(Array.from(w.stored.draOutbox,item=>item.payload.rpa_feedback.finder_decision),expected);
 assert.deepEqual(Array.from(w.stored.draOutbox,item=>item.payload.rpa_feedback.review_required),[false,false,false,true]);
 assert.equal((await saveFeiguaTestRules(w,['普通'],[])).ok,true);
 page.fingerprint='browse-new-rules';page.capturedAt=new Date(Date.now()+1000).toISOString();
 assert.equal((await send()).ok,true);
 const observations=await w.exportObservations();
 assert.equal(observations.findLast(item=>item.authorName==='热词浏览').raw.ruleDecision.code,'FEIGUA_MATCHED');
});
