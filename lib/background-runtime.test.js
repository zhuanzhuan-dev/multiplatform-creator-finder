...
const bundle = await build({
  stdin: { contents: source + '\nglobalThis.runtimeTest = { exportObservations, feigua, xingtu, kuaishou, bilibili, bilibiliPopular, initialize, watchdog, setPaused, stopRun, configureOpenTabSidePanels, flushOutbox, handleUploadAlarm, resumeRun, state: () => state };', resolveDir: root, loader: 'js' },
...
async function worker(saved = null, fetchImpl = null, connected = false) {
  const stored = saved || {
  ...
  const context = { indexedDB:new IDBFactory(),IDBKeyRange,testHistory, chrome, crypto:globalThis.crypto, structuredClone, URL, URLSearchParams, TextEncoder, TextDecoder, AbortController, setTimeout, clearTimeout, console, fetch:fetchImpl || (connected ? acceptedFixture : (()=>{throw new Error('Unexpected network call');})) };
