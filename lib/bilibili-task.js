...
export function createBilibiliRuntime(deps) {
  let state = initial();
  let seen = [];
  let creators = [];
  ...
  const surfaceOf = () => state.route?.surface === "popular" ? "popular" : "recommend";
  const valid = (tab) => {
  ...
  async function timers() {
    if (state.status === "running") {
      await chrome.alarms.create(BILIBILI_ALARM, { periodInMinutes: 0.5 });
      if (state.stopAt) await chrome.alarms.create(BILIBILI_STOP_ALARM, { when: Math.max(Date.now() + 100, state.stopAt) });
    } else {
      await chrome.alarms.clear(BILIBILI_ALARM);
      await chrome.alarms.clear(BILIBILI_STOP_ALARM);
    }
  }
  ...
  async function openPage({ tabId, windowId, focus = true, surface } = {}) {
    if (surface === "popular" || surface === "recommend") state.route = routeFor(surface);
    ...
    if (!valid(tab)) {
      ...
      if (!tab) tab = await chrome.tabs.create({ windowId: win, url: launchUrl("bilibili", surfaceOf()), active: focus });
    }
  ...
  async function launch(tab, own) {
    ...
    const route = resolveRoute(tab.pendingUrl || tab.url || "");
    if (route?.surface === "popular" || route?.surface === "recommend") state.route = routeFor(route.surface);
  ...
  function start(options = {}) {
    if (active()) return Promise.reject(Error("B站任务正在运行"));
    const surface = options.surface === "popular" ? "popular" : "recommend";
    const own = ++generation;
    return serial(async () => {
      if (active()) throw Error("B站任务正在运行");
      ...
      state = {
        ...
        freshIdx: 1,
        popularPage: 1,
        runTarget: normalizeRunTarget(deps.settings().target),
        runSettings: structuredClone(deps.settings()),
        runRules: structuredClone(deps.rules()),
        startupStage: surface === "popular" ? "正在打开 B 站综合热门" : "正在打开 B 站首页推荐"
      };
      seen = [];
      creators = [];
      await save();
      try {
        const tab = await openPage({ ...options, surface });
        return await launch(tab, own);
      } catch (error) {
  ...
  async function message(message, sender) {
    ...
    if (message.type === "DRA_TICK_ERROR") {
      await pause(message.error || "B站页面异常");
      return { ok: true };
    }
  ...
  async function inspect() {
    ...
    if (!valid(tab)) return pause(surfaceOf() === "popular" ? "B站综合热门页已关闭或离开" : "B站首页已关闭或离开");
    ...
    if (!status?.running || status.runId !== state.runId || status.loopId !== state.loopId || runtimeStalled(state.runtime)) {
      return pause("B站页面运行中断，请继续本轮");
    }
  }

  function restore(saved) {
    if (saved.draBilibili) {
      state = { ...initial(saved.draBilibili.state?.route?.surface), ...saved.draBilibili.state };
      seen = saved.draBilibili.seen || [];
      creators = saved.draBilibili.creators || [];
    }
    state.route = routeFor(state.route?.surface);
  ...
  return {
    ...
    timers,
    restore,
    storage: () => ({ draBilibili: { state, seen, creators } }),
    clear: () => {
      state = initial();
      seen = [];
      creators = [];
