...
export function mergeFeiguaPage(collection, page, keywords = DEFAULT_KEYWORDS) {
  ...
  for (const entry of decisions.accounts) {
    ...
    for (const field of ['bloggerId', 'feiguaBloggerUid', 'feiguaDetailUid', 'uid', 'secUid', 'uniqueId', 'douyinProfileUrl', 'xingtuId', 'xingtuUrl']) {
      if (row[field]) account[field] = row[field];
    }
    ...
    const decision = evaluateFeiguaRules(account.videos, {feiguaKeywords: keywords, feiguaPositiveKeywords: collection.positiveKeywords || []});
    if (JSON.stringify(account.ruleDecision) !== JSON.stringify(decision)) events.push({...decision, row});
    account.ruleDecision = decision;
  }
...
export function feiguaRecord(row, runId) {
  const decision = row.ruleDecision || evaluateFeiguaRules(row.videos || [row], {feiguaKeywords: [], feiguaPositiveKeywords: []});
  const record = buildObservationRecord({
    ...
    feedIndex: row.feedIndex, runId, isRelevant: decision.matched, decision: decision.matched ? 'keep' : 'review', action: 'collect',
    extra: { finderDecision: decision.code, reasons: decision.reasons }
  });
  Object.assign(record.rpa_feedback, {
    source: 'feigua-video-library', feigua_profile_url: row.profileUrl || '',
    feigua_blogger_id: row.bloggerId || '',
    feigua_blogger_uid: row.feiguaBloggerUid || '',
    feigua_detail_uid: row.feiguaDetailUid || '',
    unique_id: row.uniqueId || '',
    xingtu_profile_url: row.xingtuUrl || '',
    followers_text: row.followerCount || '', review_required: decision.needsReview,
    feigua_videos: row.videos || [{...row, videos: undefined}]
  });
  if (row.uid) record.uid = row.uid;
  if (row.secUid) record.sec_uid = row.secUid;
  if (row.xingtuId) record.xingtu_id = row.xingtuId;
  return record;
}
