...
export function xingtuCommercialFields(row) {
  const cpmText=String(row.metrics?.['预期CPM'] || '').replace(/,/g,'').trim();
  const cpm=/^\d+(?:\.\d+)?$/.test(cpmText) ? Number(cpmText) : null;
  return {
    cpm,
    quote_1_20s:parseCompactNumber(row.prices?.['1-20s']),
    quote_21_60s:parseCompactNumber(row.prices?.['21-60s']),
    quote_60s_plus:parseCompactNumber(row.prices?.['60s+']),
    content_tags:Array.isArray(row.tags) ? [...new Set(row.tags.filter(tag=>typeof tag==='string' && tag.trim()).map(tag=>tag.trim()))] : []
  };
}
...
export function mergeXingtuPage(collection, page, keywords = DEFAULT_KEYWORDS) {
  ...
  for (const entry of decisions.accounts) {
    ...
    if (!collection.accounts[key]) {
      ...
      events.push({ code: 'XINGTU_PENDING_REVIEW', row, reasons: ['已采集达人候选，逐页上传研究台'] });
    } else {
...
export function xingtuRecord(row, runId) {
  ...
  Object.assign(record,xingtuCommercialFields(row));
  return record;
}
