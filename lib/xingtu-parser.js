...
const AUTHOR_PATH = /^\/ad\/creator\/(?:author-homepage\/douyin-video|author(?:\/douyin)?)\/(\d{1,30})\/?$/i;
...
function extractXingtuId(url) {
  try {
    ...
    if (parsed.protocol !== 'https:' || !['www.xingtu.cn', 'xingtu.cn'].includes(parsed.hostname)) return '';
    const match = parsed.pathname.match(AUTHOR_PATH);
    if (match) return match[1];
    const queryId = parsed.searchParams.get("star_id") || "";
    return /^\d{1,30}$/.test(queryId) ? queryId : '';
  } catch { /* Malformed URLs have no verified Xingtu ID. */ }
  return "";
}

function extractXingtuDetailName(rawText, xingtuId) {
  if (!/^\d{1,30}$/.test(String(xingtuId || ''))) return '';
  const lines = String(rawText || '').split(/\r?\n/).map(cleanLine).filter(Boolean);
  const marker = lines.findIndex(line => /^星图\s*ID\s*[:：]?\s*\d{1,30}\s*$/i.test(line) && line.includes(String(xingtuId)));
  if (marker < 1) return '';
  return lines.slice(Math.max(0, marker - 4), marker).reverse()
    .find(line => line.length <= 100 && /[\p{L}\p{N}]/u.test(line)) || '';
}

function canonicalXingtuPageUrl(value) {
  try {
    const url = new URL(String(value || ''), 'https://www.xingtu.cn/');
    if (url.protocol !== 'https:' || !['www.xingtu.cn', 'xingtu.cn'].includes(url.hostname)) return '';
    const hashRoute = url.hash.split('?')[0];
    const queryId = url.searchParams.get('star_id');
    const identityQuery = !AUTHOR_PATH.test(url.pathname) && /^\d{1,30}$/.test(queryId || '') ? `?star_id=${queryId}` : '';
    return `${url.origin}${url.pathname}${identityQuery}${hashRoute}`;
  } catch { return ''; }
}
...
function parseCardText(rawText, hints) {
  ...
  const suppliedId = cleanLine(safe.xingtuId);
  const xingtuId = /^\d{1,30}$/.test(suppliedId) ? suppliedId : extractXingtuId(safe.profileUrl);
...
function rowAccountKey(row) {
  const suppliedId = cleanLine(row && row.xingtuId);
  const id = /^\d{1,30}$/.test(suppliedId) ? suppliedId : extractXingtuId(row && row.profileUrl);
...
export {
  ...
  matchKeywords,
  extractXingtuId,
  extractXingtuDetailName,
  canonicalXingtuPageUrl,
