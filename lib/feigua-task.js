...
export function createFeiguaRuntime(deps) {
  ...
  const start = tabId => { const generation=++controlGeneration; return serial(async()=>{
    ...
    if (generation !== controlGeneration) return state;
    state = {...structuredClone(INITIAL_STATE),route:{platform:'feigua',surface:'library',label:'飞瓜视频库'},status:'starting',runId:crypto.randomUUID(),loopId:crypto.randomUUID(),feedTabId:tab.id,feedWindowId:tab.windowId,backgroundMode:'current-tab',runTarget:{mode:'pages',maxPages:feiguaPolicy(deps.settings()).maxPages},batchStartPages:0,restUntil:null,elapsedMs:0,runSettings:{...structuredClone(deps.settings()),dryRun:false},runRules:structuredClone(deps.rules())};
    feiguaCollection = newFeiguaCollection(state.runId,state.runSettings.dryRun); feiguaCollection.keywords=[...state.runRules.feiguaKeywords]; feiguaCollection.positiveKeywords=[...(state.runRules.feiguaPositiveKeywords || [])];
    await save();
  ...
  const resume = (options={}) => { const generation=++controlGeneration; return serial(async()=>{
    ...
    await deps.authorize();
    state.runSettings = {...state.runSettings,dryRun:false};
    if (feiguaCollection) feiguaCollection.dryRun=false;
    if (generation !== controlGeneration) return state;
  ...
  async function finish(reason='用户结束本轮') {
    if (state.stats.pageReadTimeouts) reason += `（有 ${state.stats.pageReadTimeouts} 次页面读取超时，未计入成功采集页数）`;
    cancel(reason);
    await timers();
    ...
    await save(); void deps.scheduleUpload(true); return state;
  }
  ...
  async function message(message,sender) {
    ...
    return serial(async()=>{
      ...
      if (message.type === 'DRA_FEIGUA_PAGE_TIMEOUT') {
        const route = resolveRoute(message.pageUrl || '');
        if (route?.platform !== 'feigua' || route.surface !== 'library') throw Error('飞瓜采集来源不匹配');
        increment('pageReadTimeouts');
        state.runtime={phase:'transition',progressAt:Date.now()};
        deps.recordDecision({code:'FEIGUA_PAGE_TIMEOUT',reasons:['等待 30 秒后页面仍未更新或稳定，尝试翻下一页；本次未计为采集成功']},
          {authorName:`飞瓜第 ${String(message.pageNumber || '未知').slice(0,20)} 页`,beforeUrl:message.pageUrl,sourcePlatform:'feigua'},null,state);
        await save();
        return {ok:true};
      }
  ...
  return {state:()=>state,collection:()=>feiguaCollection,restore,start,resume,stop,pause,message,openPage,noteSchedule,inspect:()=>serial(inspect),timers,active,storage:()=>({draFeiguaState:state,draFeiguaCollection:feiguaCollection}),clear:()=>{feiguaCollection=null;},increment};
async function handleFeiguaPage(page, runId, loopId) {
  ...
  const classifiedPage = {...page, rows: page.rows.map(row => {
    const key = `feigua:${rowAccountKey(row)}`;
    const decision = copy.excluded[key]
      ? {code:'FEIGUA_EXCLUDED',matched:false,needsReview:false,reasons:[`命中负向排除词：${copy.excluded[key].matchedKeywords.join('、')}`]}
      : copy.accounts[key]?.ruleDecision;
    return {...row, ruleDecision: decision};
  })};
  await deps.savePage(classifiedPage,runId);
  ...
  increment("scanned", result.fresh.length);
  ...
  state.stats.reviewQueued = Object.values(copy.accounts).filter(row => row.ruleDecision?.needsReview !== false).length;
  state.stats.ruleMatched = Object.values(copy.accounts).filter(row => row.ruleDecision?.matched).length;
  ...
  for (const row of result.fresh) deps.recordScan( `feigua:${row.videoId || row.authorName + row.title}`);
...
async function finalizeFeiguaCollection() {
  ...
  // Raw local observations are durable; only the preview queue is bounded.
  if (!collection.dryRun) deps.reserveQueue(entries.length);
