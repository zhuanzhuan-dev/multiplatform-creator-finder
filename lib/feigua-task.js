...
export function createFeiguaRuntime(deps) {
  ...
  const start = tabId => { const generation=++controlGeneration; return serial(async()=>{
    ...
    if (generation !== controlGeneration) return state;
    state = {...structuredClone(INITIAL_STATE),route:{platform:'feigua',surface:'library',label:'飞瓜视频库'},status:'starting',runId:crypto.randomUUID(),loopId:crypto.randomUUID(),feedTabId:tab.id,feedWindowId:tab.windowId,backgroundMode:'current-tab',runTarget:{mode:'pages',maxPages:feiguaPolicy(deps.settings()).maxPages},batchStartPages:0,restUntil:null,elapsedMs:0,runSettings:{...structuredClone(deps.settings()),dryRun:deps.settings().dryRun},runRules:structuredClone(deps.rules())};
  ...
  const resume = (options={}) => { const generation=++controlGeneration; return serial(async()=>{
    ...
    if (deps.settings().dryRun || feiguaCollection?.dryRun) {
