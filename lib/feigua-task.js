...
export function createFeiguaRuntime(deps) {
  ...
  async function openPage({windowId}={}) {
    ...
    if (!tab) tab=await chrome.tabs.create({windowId:destinationWindowId,url:launchUrl('feigua','library'),active:true});
    else if (!resolveRoute(tab.pendingUrl || tab.url || '')?.runnable) tab=await chrome.tabs.update(tab.id,{url:launchUrl('feigua','library'),active:true});
  ...
  const resume = (options={}) => { const generation=++controlGeneration; return serial(async()=>{
    ...
    try {
      const tab=await openPage(options);
      if (generation !== controlGeneration) return state;
