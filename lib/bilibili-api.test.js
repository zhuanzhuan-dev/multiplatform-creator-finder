...
test("enrichMatchedBilibili pulls view and followers without author archives", async () => {
  resetWbiCache();
  const calls = [];
  const fetchImpl = mockBilibiliFetch({
    ...
    "/x/space/wbi/arc/search": () => {
      calls.push("space");
      return jsonOk({ code: 0, data: { list: { vlist: [] }, page: { count: 80 } } });
    }
  });
  ...
  assert.deepEqual(calls, ["view", "followers"]);
  assert.equal(result.followers, 9876);
  assert.deepEqual(result.archives, []);
  assert.equal(result.archiveCount, null);
