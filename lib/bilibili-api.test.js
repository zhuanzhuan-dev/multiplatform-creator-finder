...
test("enrichMatchedBilibili pulls view, followers and only the first 30 archives", async () => {
  ...
  const fetchImpl = mockBilibiliFetch({
    "/x/web-interface/wbi/view": () => {
      ...
      return jsonOk({
        ...
        data: {
          ...
          pages: [{ cid: 9, page: 1, part: "P1" }]
        }
      });
    },
    "/x/relation/stat": () => {
      calls.push("followers");
      return jsonOk({ code: 0, data: { mid: 12345, follower: 9876, following: 12 } });
    },
    "/x/space/wbi/arc/search": (parsed) => {
      ...
      return jsonOk({
        ...
        data: {
          list: {
            vlist: Array.from({ length: BILIBILI_SPACE_PAGE_SIZE }, (_, index) => make(`a${index}`)),
            tlist: { 76: { tid: 76, count: 3, name: "美食" } }
          },
          episodic_button: { text: "播放全部", uri: "https://www.bilibili.com/list/12345" },
          is_risk: false,
          gaia_data: { token: "skip" },
  ...
  assert.equal(result.observation.viewDetail.pages[0].cid, 9);
  assert.equal(result.observation.relationStat.following, 12);
  assert.equal(result.observation.authorArchivePartitions[76].name, "美食");
  assert.equal(result.observation.authorEpisodicButton.text, "播放全部");
  assert.equal(result.observation.authorArchives.some((item) => item.gaia_data), false);
...
test("clipBilibiliObservation drops the season list before the feed card", () => {
  const clipped = clipBilibiliObservation({
    videoId: "BV1xx411c7mD",
    raw: { bvid: "BV1xx411c7mD", track_id: "keep-me" },
    viewDetail: { bvid: "BV1xx411c7mD", ugc_season: { title: "合集", blob: "z".repeat(95_000) } },
    authorArchives: [{ videoId: "BV1a", description: "keep" }]
  });
  assert.equal(clipped.viewDetail.ugc_season, null);
  assert.deepEqual(clipped.viewDetailClipped, ["ugc_season"]);
  assert.equal(clipped.raw.track_id, "keep-me");
...
test("bilibili upload keeps the feed card, view payload and relation stat", () => {
  const card = mapBilibiliRecommendItem({
    goto: "av",
    bvid: "BV1xx411c7mD",
    title: "测试视频",
    pic: "https://example.com/cover.jpg",
    ...
    uri: "https://www.bilibili.com/video/BV1xx411c7mD",
    owner: { mid: 12345, name: "UP主", face: "https://example.com/face.jpg" },
    stat: { view: 1000, like: 20, danmaku: 3 },
    track_id: "trace-1",
    rcmd_reason: { content: "已关注", reason_type: 1 }
  });
  const fields = bilibiliFeedbackFields({
    ...card,
    viewDetail: { bvid: "BV1xx411c7mD", pages: [{ cid: 9 }], stat: { coin: 4 } },
    relationStat: { follower: 10, following: 2 },
    authorArchivePartitions: { 76: { name: "美食" } }
  });
  assert.equal(fields.feed_item.track_id, "trace-1");
  assert.equal(fields.feed_item.rcmd_reason.reason_type, 1);
  assert.equal(fields.view_detail.pages[0].cid, 9);
  assert.equal(fields.relation_stat.following, 2);
  assert.equal(fields.author_archive_partitions[76].name, "美食");
});
