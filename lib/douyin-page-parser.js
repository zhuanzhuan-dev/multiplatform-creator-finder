(function attachDouyinParser(global) {
  ...
  function creatorDrawerRoot(documentRef = document, scope = visibleFeedItem(documentRef)) {
    ...
    while (element?.parentElement) {
      ...
      if (/\d+(?:\.\d+)?(?:万|亿|w|k)?\s*粉丝/i.test(text)
        ...
        && (directWorksList(element) || element.querySelector('a[href*="/video/"], img[src]'))) return element;
  ...
  function parseFeedItem(element, documentRef = document) {
    ...
    const subtype = element.querySelector('[data-e2e^="feed-"]')?.getAttribute("data-e2e") || "";
    ...
    const caption = captionCandidates.find((line) => line.includes("#"))
    ...
    const durationSeconds = Number.isFinite(displayedDurationSeconds) && displayedDurationSeconds > 0
      ...
      : Number.isFinite(mediaDurationSeconds) && mediaDurationSeconds > 0
        ...
        : null;
    const likeNode = element.querySelector('[data-e2e="like-count"], [data-e2e="video-like-count"], [aria-label*="点赞"]');
    ...
    return {
      ...
      likesText,
      isLive: subtype === "feed-live" || /直播中|进入直播间/.test(cardText),
      subtype,
  ...
  function parseCreatorPanel(documentRef = document, observation = {}) {
    ...
    const media = [
      ...list.querySelectorAll('a[href*="/video/"] img[src]'),
      ...list.querySelectorAll('img[src]')
    ...
    }).filter((item) => item.coverAlt || Number.isFinite(item.likes)).slice(0, 24);
