(function attachDouyinParser(global) {
  ...
  function avatarUrlFromImage(image) {
    return coverUrlFromImage(image);
  }
  ...
  function avatarUrlForProfile(root, profileUrl = "", authorName = "") {
    ...
    const directAvatar = root.querySelector('[data-e2e="video-avatar"] img');
    const directUrl = avatarUrlFromImage(directAvatar);
  ...
  function creatorPanelState(documentRef = document) {
    ...
    const loading = Boolean(list?.closest('[aria-busy="true"]')) || [...(list?.querySelectorAll('[aria-busy="true"], [role="progressbar"]') || [])]
      .some(element => visibleArea(element, documentRef) > 0);
    return {
      present: Boolean(tab),
      selected,
      hasWorks,
      loading,
      ready: Boolean(tab && list && hasWorks && !loading && (selected || tab.getAttribute("role") !== "tab"))
    };
  }
  ...
  function feedContentIdentity(element, drawer) {
    ...
    // Author live badges/entry links also appear on ordinary video cards.
    // Require a content marker or a live-room link wrapping the actual player.
    const contentNode = (node) => !node.closest('[data-e2e="video-desc"], [data-e2e="feed-video-nickname"], [data-e2e="feed-video-avatar"]');
    const liveMarker = nodes.some((node) => contentNode(node) && node.getAttribute('data-e2e') === 'feed-live');
    const livePlayer = nodes.some((node) => contentNode(node) &&
      /^https:\/\/live\.douyin\.com\/\d+(?:[/?#]|$)/.test(node.href || '') &&
      [...node.querySelectorAll('video, [data-e2e="image-player"]')].some(visible));
    const live = liveMarker || livePlayer;
    ...
    // Recommendation cards default to video across different player implementations.
    // Only positive live/photo evidence overrides this default; ad stays independent.
    const visibleVideo = nodes.some((node) => node.tagName === 'VIDEO');
    const contentType = live ? 'live' : photo ? 'photo' : 'video';
    const typeEvidence = [];
    if (live) typeEvidence.push(liveMarker ? 'visible marker: feed-live' : 'live-room link wraps visible player');
    ...
    if (contentType === 'video') typeEvidence.push(has('feed-video') ? 'visible marker: feed-video' : visibleVideo ? 'visible video element' : 'recommendation card defaults to video');
    ...
    return { contentType, isLive: live, isAd, typeEvidence, subtype: live ? 'feed-live' : photo ? 'feed-photo' : 'feed-video' };
  }
  ...
  function parseCreatorPanel(documentRef = document, observation = {}) {
    ...
    return {
      ready: Boolean(matchesObservation && authorName && Number.isFinite(parseCompactNumber(followersText)) && recentVideos.length),
