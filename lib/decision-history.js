...
export function historyTone(code = '') {
  ...
  if (value==='ACCEPTED' || value==='FEIGUA_MATCHED') return 'good';
...
export function normalizeHistoryEntry(entry, fallbackPlatform = 'unknown') {
  ...
  return {
    ...
    tname:String(entry.tname || '').slice(0,80),
...
export function historyMatches(entry, {sourcePlatform='',runId='',query='',authorId='',profileUrl=''} = {}) {
  ...
  return !term || [entry.accountName,entry.tname,entry.code,entry.caption,entry.videoId,...entry.reasons].join(' ').toLocaleLowerCase('zh-CN').includes(term);
}
