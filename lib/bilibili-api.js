...
export async function enrichMatchedBilibili(observation, {
  ...
  // 作者投稿已暂停（2026-09-23）。侧栏「视频」读的是本机已采集记录，不依赖这份列表。
  // 恢复时去掉下面块注释，并把 return 里的空投稿换成注释中的 archives 等变量。
  // 暂停前的顺序：粉丝数之后再请求 /x/space/wbi/arc/search（pn=1, ps=30，按发布时间）。当时这次请求前另有 8–16 秒等待，现已一并去掉。
  // 写入 authorArchives（最新 30 条）、authorArchiveCount（总数）、authorArchivesComplete、
  // authorArchivePartitions（tlist）、authorEpisodicButton。规格见「作者投稿补抓已暂停」。
  /*
  let archives = [];
