export function launcherState(state = {}) {
  ...
  const tasks = (state.tasks || []).filter(task => ['starting','running','paused'].includes(task.status)).map(task => ({
    platform:task.route?.platform === 'bilibili' && task.route?.surface === 'popular' ? 'bilibili-popular' : task.route?.platform,
    name:task.route?.platform === 'bilibili' ? (task.route?.surface === 'popular' ? 'B站热门' : 'B站推荐') : ({douyin:'抖音',kuaishou:'快手',feigua:'飞瓜',xingtu:'星图'}[task.route?.platform] || '任务'),
    status:task.status, label:launcherState(task).label, count:Number(task.stats?.scanned || 0),
    unit:['feigua','xingtu'].includes(task.route?.platform) ? '页' : '条'
  }));
