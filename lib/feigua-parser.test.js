...
test("同一账号任一视频命中规避词时排除整个账号", () => {
  const profileUrl = "https://dy.feigua.cn/app/#/blogger-detail/index?bloggerId=42";
  const rows = [
    { authorName: "测试达人", profileUrl, title: "普通美食分享", topics: [], hotWords: [], searchText: "普通美食分享 测试达人" },
    { authorName: "测试达人", profileUrl, title: "今日社会百态记录", topics: [], hotWords: [], searchText: "今日社会百态记录 测试达人" }
  ];
...
test("同一账号多条未命中视频只生成一个账号", () => {
  const profileUrl = "https://dy.feigua.cn/app/#/blogger-detail/index?bloggerId=42";
  const rows = [
    { authorName: "账号甲", profileUrl, followerCount: "12.6w", title: "视频一", topics: [], hotWords: [], searchText: "视频一 账号甲" },
    { authorName: "账号甲", profileUrl, followerCount: "12.6w", title: "视频二", topics: [], hotWords: [], searchText: "视频二 账号甲" }
  ];
...
test("同名账号即使主页链接参数变化仍视为重复账号", () => {
  const rows = [
    { authorName: "账号甲", profileUrl: "https://dy.feigua.cn/app/#/blogger-detail/index?bloggerId=42&from=a", title: "视频一", searchText: "视频一 账号甲" },
    { authorName: "账号甲", profileUrl: "https://dy.feigua.cn/app/#/blogger-detail/index?bloggerId=42&from=b", title: "视频二", searchText: "视频二 账号甲" }
  ];
...
test("缺少飞瓜达人 ID 的同名行不合并成达人", () => {
  const rows = [
    { authorName: "同名账号", title: "视频一", searchText: "视频一 同名账号" },
    { authorName: "同名账号", title: "视频二", searchText: "视频二 同名账号" }
  ];
  const result = core.decideRows(rows, []);
  assert.equal(result.accounts.length, 0);
