...
test('launcher follows panel presence and ends its completion notice',()=>{
 const state={status:'running',stats:{scanned:19}};
 assert.equal(launcherState(state,false).visible,true);
 assert.equal(launcherState(state,true).visible,false);
 ...
 assert.equal(launcherState({...state,status:'stopped',endedAt:new Date(10000).toISOString()},false,19000).visible,false);
 assert.equal(launcherState({},false).visible,false);
