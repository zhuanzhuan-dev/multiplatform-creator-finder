import { DEFAULT_KEYWORDS as FEIGUA_KEYWORDS, parseKeywords } from "./feigua-parser.js";
import { DEFAULT_KEYWORDS as XINGTU_KEYWORDS, parseKeywords as parseXingtuKeywords } from "./xingtu-parser.js";
import { DEFAULT_ENGAGEMENT, newEngagement } from "./engagement.js";
import { numericValue } from "./numeric.js";

export const STORAGE_KEYS = Object.freeze({
  feiguaState: "draFeiguaState",
  feiguaCollection: "draFeiguaCollection",
  xingtuState: "draXingtuState",
  xingtuCollection: "draXingtuCollection",
  settings: "draSettings",
  settingsDraft: "draSettingsDraft",
  rules: "draRules",
  state: "draState",
  dailyStats: "draDailyStats",
  decisionHistory: "draDecisionHistory",
  debugSettings: "draDebugSettings",
...
export const DEFAULT_SETTINGS = Object.freeze({
  ...
  dryRun: false,
  feiguaTarget: { maxPages: 50 },
  feiguaDelay: { minSeconds: 4, maxSeconds: 6 },
  xingtuUploadEnabled: false,
  xingtuTarget: { maxPages: 50 },
  xingtuDelay: { minSeconds: 4, maxSeconds: 6 },
  bilibiliBatchDelay: { minSeconds: 5, maxSeconds: 12 },
  schedule: {
    enabled: true,
    weekdays: [2, 3, 4, 5, 6],
...
export function mergeSettings(value = {}) {
  const { feiguaUploadEnabled: _retiredFeiguaUploadGate, ...current } = value;
  value = current;
  ...
  const dwell = {
    ...
    ...(value.dwell || {}),
    ...(!value.dwell && Number.isFinite(legacyDwellSeconds) ? { mode: "fixed", fixedSeconds: legacyDwellSeconds } : {})
  };
  const target = {
  ...
  const bilibiliBatchDelay = {
    ...DEFAULT_SETTINGS.bilibiliBatchDelay,
    ...(value.bilibiliBatchDelay || {})
  };
  const xingtuTarget = {
    ...DEFAULT_SETTINGS.xingtuTarget,
    ...(value.xingtuTarget || {})
  };
  const xingtuDelay = {
    ...DEFAULT_SETTINGS.xingtuDelay,
    ...(value.xingtuDelay || {})
  };
  return {
    ...DEFAULT_SETTINGS,
    ...
    xingtuTarget,
    xingtuDelay,
    engagement: { ...DEFAULT_ENGAGEMENT, ...(value.engagement || {}) },
    schedule: {
...
export function validateSettings(settings = {}) {
  const problems = [];
  const engagement = settings.engagement || DEFAULT_ENGAGEMENT;
  if (!Number.isInteger(engagement.rate) || engagement.rate < 0 || engagement.rate > 100) problems.push("互动配额必须是 0–100 的整数百分比");
  if (["like", "collect", "follow"].some(action => typeof engagement[action] !== "boolean")) problems.push("互动开关必须为勾选状态");
  const dwell = settings.dwell || {};
  if (!new Set(["fixed", "range"]).has(dwell.mode)) problems.push("停留模式必须是固定或随机区间");
  if (dwell.mode === "fixed") {
    const fixed = numericValue(dwell.fixedSeconds);
    if (!Number.isFinite(fixed) || fixed < 5 || fixed > 300) problems.push("固定停留必须在 5–300 秒之间");
  }
  if (dwell.mode === "range") {
    const min = numericValue(dwell.minSeconds);
    const typical = numericValue(dwell.typicalSeconds);
    const max = numericValue(dwell.maxSeconds);
    if (![min, typical, max].every(Number.isFinite) || min < 5 || max > 300 || min > typical || typical > max) {
      problems.push("随机停留必须满足 5 ≤ 最短 ≤ 常见 ≤ 最长 ≤ 300 秒");
    }
  }
  const batchMin = numericValue(settings.bilibiliBatchDelay?.minSeconds);
  const batchMax = numericValue(settings.bilibiliBatchDelay?.maxSeconds);
  if (settings.bilibiliBatchDelay != null || Number.isFinite(batchMin) || Number.isFinite(batchMax)) {
    if (![batchMin, batchMax].every(Number.isFinite) || batchMin < 1 || batchMax > 300 || batchMin > batchMax) {
      problems.push("B站批间间隔必须满足 1 ≤ 最短 ≤ 最长 ≤ 300 秒");
    }
  }
  ...
  for (const [label, target] of targets) {
    ...
    const duration = numericValue(target?.durationMinutes);
    const count = numericValue(target?.maxItems);
...
export function scheduleFrom(raw, optedIn = false) {
  const schedule = mergeSettings({ schedule: raw || { enabled: false } }).schedule;
  return optedIn ? schedule : { ...schedule, enabled: false };
}

export function mergeXingtuSettings(value = {}, { scheduleOptIn = false } = {}) {
  const pages = Number(value.xingtuTarget?.maxPages);
  const min = Number(value.xingtuDelay?.minSeconds);
  const max = Number(value.xingtuDelay?.maxSeconds);
  return {
    dryRun: Boolean(value.dryRun),
    keepSystemAwake: value.keepSystemAwake !== false,
    xingtuUploadEnabled: value.xingtuUploadEnabled === true,
    xingtuTarget: { maxPages: Number.isInteger(pages) && pages >= 1 && pages <= 500 ? pages : 50 },
    xingtuDelay: {
    ...
    schedule: scheduleFrom(value.schedule, scheduleOptIn)
  };
}

export function mergeXingtuRules(value = {}) {
  return {
    xingtuKeywords: Array.isArray(value.xingtuKeywords) ? parseXingtuKeywords(value.xingtuKeywords) : [...XINGTU_KEYWORDS]
  };
}
...
export function mergeRules(value = {}) {
  return {
    ...
    feiguaPositiveKeywords: Array.isArray(value.feiguaPositiveKeywords) ? parseKeywords(value.feiguaPositiveKeywords) : [],
    feiguaKeywords: Array.isArray(value.feiguaKeywords) ? parseKeywords(value.feiguaKeywords) : [...FEIGUA_KEYWORDS],
    xingtuKeywords: Array.isArray(value.xingtuKeywords) ? parseXingtuKeywords(value.xingtuKeywords) : [...XINGTU_KEYWORDS],
